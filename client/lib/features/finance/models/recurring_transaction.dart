...
class RecurringTransactionUpdateRequest {
  ...
  /// Fields the user explicitly cleared in the edit form.
  ///
  /// The backend uses ``exclude_unset`` semantics: a key that is present with
  /// a null value clears the field, while an absent key keeps the old value.
  /// Dart cannot distinguish "not provided" from "null" on nullable fields,
  /// so cleared fields are tracked explicitly and serialized as explicit nulls.
  final Set<UpdateClearableField> clearedFields;
  ...
  Map<String, dynamic> toJson() {
    ...
    // Explicit nulls for cleared fields so the backend actually resets them.
    for (final field in clearedFields) {
      json[field.wireName] = null;
    }
    return json;
  }
}

/// Nullable fields of a recurring transaction that can be explicitly cleared
/// during an update. See [RecurringTransactionUpdateRequest.clearedFields].
enum UpdateClearableField {
  ...
  const UpdateClearableField(this.wireName);
}

/// A pending transaction generated by a recurring rule that requires user confirmation.
