...
class BudgetService:
    ...
    def __init__(self, session: AsyncSession):
        ...
        self.periods = BudgetPeriodService(session)
    ...
    async def get_user_budgets_with_periods(
        self,
        user_uuid: UUID,
        status: BudgetStatus | None = None,
        scope: BudgetScope | None = None,
    ) -> list[BudgetResponse]:
        """Fetch user budgets with their current period spent amounts.

        Wraps ``get_user_budgets`` + period refresh in one transaction so the
        route layer never reaches into the session directly (m10).

        Returns:
            List of budget responses (via ``build_budget_response``).
        """
        budgets = await self.get_user_budgets(user_uuid, status=status, scope=scope)
        responses = []
    ...
    async def _get_current_period(self, budget: Budget) -> BudgetPeriod | None:
        ...
        return await self.periods._get_current_period(budget)
    ...
    async def get_or_create_current_period(self, budget: Budget) -> BudgetPeriod:
        ...
        """
        return await self.periods.get_or_create_current_period(budget)
    ...
    def _calculate_period_range(
        ...
        """
        return self.periods._calculate_period_range(period_type, anchor_day, reference_date)
    ...
    async def calculate_spent_amount(
        ...
        Returns:
            Total spent amount
        """
        return await self.periods.calculate_spent_amount(user_uuid, period_start, period_end, category_key)
    ...
    async def update_period_spent_amount(
        ...
        *,
        ...
        """
        return await self.periods.update_period_spent_amount(budget, period, auto_commit=auto_commit)
