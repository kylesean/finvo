...
class TransactionService:
    ...
    async def confirm_pending_transaction(self, transaction_id: UUID, user_uuid: UUID) -> dict[str, Any]:
        """Confirm a PENDING transaction (status -> CONFIRMED)."""
        return await self._crud.confirm_pending_transaction(transaction_id, user_uuid)

    async def skip_pending_transaction(self, transaction_id: UUID, user_uuid: UUID) -> None:
        """Skip (delete) a PENDING transaction."""
        await self._crud.skip_pending_transaction(transaction_id, user_uuid)

    async def list_pending_transactions(self, user_uuid: UUID) -> list[dict[str, Any]]:
        """List PENDING transactions for a user (auto-generated by recurring rules)."""
        return await self._crud.list_pending_transactions(user_uuid)
