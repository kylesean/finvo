...
def format_messages(messages: list[dict]) -> str:
    ...
    for idx, message in enumerate(messages):
        ...
        if msg_type == "tool":
            ...
            # Safely try to get tool call arguments from previous message
            tool_input = "unknown"
            if idx > 0:
                prev_msg = messages[idx - 1]
                tool_calls = prev_msg.get("additional_kwargs", {}).get("tool_calls", [])
                if tool_calls and len(tool_calls) > 0:
                    tool_input = tool_calls[0].get("function", {}).get("arguments", "unknown")

            content_str = str(msg_content)
            if len(content_str) > 100:
                formatted_messages.append(f"tool {tool_name} input: {tool_input} {content_str[:100]}...")
            else:
                formatted_messages.append(f"tool {tool_name} input: {tool_input} output: {content_str}")
        elif msg_content:
